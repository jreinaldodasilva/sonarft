...
| 2 | `trade_position` unbound variable (neutral direction) | Trading Logic | **Critical** | Crash on ~30% of market conditions | Prompt 03 |
...
| 6 | `weighted_adjust_prices` returns 2-tuple on failure | Async/Trading | **High** | ValueError crash per indicator failure | Prompt 02/03 |
| 7 | Medium volatility threshold ÷100 bug | Trading Logic | **High** | ~80% of trades blocked silently | Prompt 03 |
...
| 11 | `order_placed` not checked for `None` | Exchange Integration | **High** | TypeError crash + untracked order | Prompt 06 |
| 12 | StochRSI parameter mismatch (RSI period = 3) | Indicators | **High** | Excessive false signals | Prompt 05 |
| 13 | `previous_spread` race condition | Async/Indicators | **High** | Incorrect spread rate under concurrency | Prompt 02 |
| 14 | `self.volatility` race condition | Async/Validators | **High** | Wrong volatility classification | Prompt 02 |
| 15 | WebSocket disconnect infinite loop | Async | **High** | Dead socket loop on disconnect | Prompt 02 |
| 16 | `np.mean/std` on empty list → NaN thresholds | Financial Math | **High** | All trades silently blocked | Prompt 04 |
| 17 | `bid_prices[0]` IndexError on empty order book | Financial Math | **High** | Crash on empty market | Prompt 04 |
| 18 | Profit threshold 0.01% below break-even | Configuration | **High** | Systematic losses in live trading | Prompt 03 |
| 19 | No API authentication by default | Security | **High** | All endpoints publicly accessible | Prompt 08 |
| 20 | `get_short_term_market_trend` NameError on zero prices | Indicators | **High** | Crash on zero-price data | Prompt 05 |
...
|---|---|---|---|---|
| 1 | Fix `trade_position` unbound variable | Trading Logic | 30 min | Yes |
| 2 | Fix `weighted_adjust_prices` return arity | Trading Logic | 15 min | Yes |
| 3 | Fix StochRSI positional parameter mismatch | Indicators | 15 min | No (but corrupts signals) |
| 4 | Fix WebSocket disconnect infinite loop | Async | 30 min | No |
| 5 | Fix Medium volatility threshold `/ 100` bug | Trading Logic | 15 min | No (but blocks most trades) |
...
| 10 | Fix `order_placed` None check in `execute_order` | Exchange | 15 min | Yes |
| 11 | Fix `get_last_price` None check in `monitor_price` | Exchange | 15 min | Yes |
...
| 18 | Fix `get_short_term_market_trend` NameError | Indicators | 15 min | No |
| 19 | Add `SONARFT_API_TOKEN` requirement + startup warning | Security | 1 hour | No |
